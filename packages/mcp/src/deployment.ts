...
export function predictRegistrar(
  ...
  return predictNamespaceContract(registry, node, nonce, passphrase, version, 1);
}

export function predictResolver(
  registry: string, node: Uint8Array, nonce: Uint8Array,
  passphrase: string, version: 0 | 1,
): string {
  return predictNamespaceContract(registry, node, nonce, passphrase, version, 2);
}

function predictNamespaceContract(
  registry: string, node: Uint8Array, nonce: Uint8Array,
  passphrase: string, version: 0 | 1, role: 1 | 2,
): string {
  ...
  if (version === 1) {
    ...
    bound.set(domain); bound[domain.length] = role;
